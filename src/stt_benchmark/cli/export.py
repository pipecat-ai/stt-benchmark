...
def export_service_data(
    service: str = typer.Option(
        ...
        "--service",
        "-s",
        help="Service name to export data for",
    ),
    ...
    test: bool = typer.Option(
        ...
        "--test",
        "-t",
        help="Export from test database (test_results.db) instead of main database",
    ),
):
    ...
    if test:
        console.print("[yellow]Exporting from test database[/yellow]")
    ...
    asyncio.run(_export_data(service_name, model, output_dir, format, db_path))
